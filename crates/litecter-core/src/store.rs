...
impl Store {
    ...
    pub fn add_url(
        ...
        self.conn
            .execute(
                "INSERT INTO urls (url, schedule, selector, next_check_at, created_at) \
                 VALUES (?1, ?2, ?3, ?4, ?4)",
    ...
    pub fn remove_url(&self, id: i64) -> Result<()> {
    ...
    pub fn set_schedule(&self, id: i64, schedule: Schedule, now: i64) -> Result<()> {
        ...
        self.conn.execute(
            "UPDATE urls SET schedule = ?2, next_check_at = ?3 WHERE id = ?1",
            params![id, schedule.to_string(), next],
        )?;
        Ok(())
    }
        ...
        if updated > 0 {
            self.conn.execute(
                "UPDATE urls SET last_seen_snapshot_id = \
                     (SELECT to_snapshot_id FROM changes WHERE id = ?1) \
